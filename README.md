...
- Firebase 인증을 활용한 자동 회원가입과 로그인 과정의 동시성 이슈
  ...
  인증 과정에서 중복 회원가입으로 인한 API 실행 에러가 발생. 중복 문제는 유니크 제약조건을 적용하여 간단히 해결할 수 있었지만, 이 과정에서 트랜잭션이 적용되지 않은 코드를 발견. 자동 회원가입과 로그인 구현을 위해 트랜잭션을 적용했으나, 원자성이 제대로 보장되지 않아 문제가 지속됨. 문제 상황을 재구현하고 디버깅을 통해 분석한 결과, Spring AOP의 트랜잭션 처리와 애플리케이션/DB 원자성에 대한 이해가 부족했던 것이 원인. 해결책으로 유니크 제약 조건을 적용하고 DataIntegrityViolationException이 발생한 경우 재시도 로직을 추가. 그러나 해당 예외는 유니크 제약 조건 위반이 아닌 경우에도 발생할 수 있어 의도하지 않은 재시도 로직이 수행될 수 있음을 인지. 따라서 API 호출 프로세스를 분리하도록 변경하여 문제 해결.
